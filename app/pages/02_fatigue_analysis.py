...
fat_class = st.sidebar.number_input(t("fat_class"), value=default_fat, min_value=36, max_value=225)
if recommended:
    st.sidebar.caption(f"{t('recommended')} : FAT {recommended.fat_class} - {recommended.description}")
...
with st.sidebar.expander(t("method_explanation")):
    _help_keys = {"nominal": "help_nominal", "hotspot": "help_hotspot", "notch": "help_notch"}
...
# ── Helpers for translated plot labels ────────────────────────────────────
def _sn_labels():
    return {
...
if "fea_hotspot_stress" in st.session_state:
    ...
    with st.container(border=True):
        ...
        with bcol1:
            ...
            st.markdown(t("fea_hotspot_detected", stress=f"{hs:.1f}", hs_type=hs_type))
        with bcol2:
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    t("single_load_block"), t("variable_amplitude_miner"),
    t("sn_curve_explorer"), t("fat_class_catalog"), t("haigh_diagram"),
])
...
with tab1:
    ...
    with col1:
        stress_range = st.number_input(
            t("stress_range"), value=_prefill_stress, min_value=0.1, step=1.0,
            help=t("help_stress_range"),
        )
        num_cycles = st.number_input(
            t("number_of_cycles"), value=2_000_000, min_value=1, step=1000,
    ...
    # ── S-N curve plot (full width, ABOVE summary) ────────────────────────
    ...
    fig = FatiguePlots.sn_curve_plotly(
        sn, op_point,
        title=t("plot_sn_title", fat=sn.fat_class, mat=format_family(sn.material_type)),
        labels=_sn_labels(),
    )
    st.plotly_chart(fig, key="sn_tab1")

    # ── Rich Engineering Summary Panel (BELOW plot) ───────────────────────
    if "fatigue_result" in st.session_state:
        r = st.session_state["fatigue_result"]["single_block_result"]
        D = r["damage_ratio"]
        sf = r["safety_factor"]
        ...
        stress_margin = (allowable_stress - stress_range) if allowable_stress > 0 else float("inf")
        ...
        # ── What-If Calculator ────────────────────────────────────────────
        with st.expander(f"🔬 {t('what_if_calculator')}", expanded=False):
            wi_stress = st.slider(
            ...
            wi_D = num_cycles / wi_N if wi_N != float("inf") and wi_N > 0 else 0.0
            ...
            _wi_sf = f"SF = {wi_sf:.2f}" if wi_sf != float("inf") else "SF = ∞"
            if wi_N == float("inf"):
                _wi_nf = "∞"
            elif wi_N < 1e9:
                _wi_nf = f"{int(wi_N):,}"
            ...
            st.markdown(
                ...
                f'<span>Δσ = {wi_stress:.0f} MPa → {wi_status}</span>'
                f'<span>D = {wi_D:.4f}  |  {_wi_sf}  |  N = {_wi_nf}</span></div>',
                unsafe_allow_html=True,
            )


# ══════════════════════════════════════════════════════════════════════════
# TAB 2 — Miner (Variable Amplitude)
# ══════════════════════════════════════════════════════════════════════════
with tab2:
    st.subheader(t("miner_assessment"))

    # ── Rainflow signal upload ────────────────────────────────────────────
    with st.expander(t("rainflow_from_signal"), expanded=False):
        st.markdown(t("rainflow_explanation"))
        sig_file = st.file_uploader(t("upload_time_signal"), type=["csv", "txt"], key="rf_upload")
        if sig_file:
            import pandas as pd
            ...
            if len(sig_df.columns) > 1:
                sig_col = st.selectbox(t("signal_column"), sig_df.columns)
            signal = sig_df[sig_col].values
            n_bins = st.slider(t("rainflow_bins"), 8, 128, 64, key="rf_bins")
            rf_spectrum = signal_to_spectrum(signal, n_bins)
            rf_df = pd.DataFrame(rf_spectrum, columns=[t("stress_range"), t("number_of_cycles")])
            st.dataframe(rf_df, hide_index=True)
            import plotly.express as px
            fig_rf = px.bar(rf_df, x=t("stress_range"), y=t("number_of_cycles"),
    ...
    # Use rainflow spectrum if available
    rf_loaded = st.session_state.get("rainflow_spectrum")
    if rf_loaded:
        n_blocks = len(rf_loaded)
        st.info(f"Rainflow: {n_blocks} blocks loaded")
    else:
        n_blocks = st.number_input(t("number_load_blocks"), value=3, min_value=1, max_value=20)

    spectrum = []
    for i in range(n_blocks):
        c1, c2 = st.columns(2)
        default_sr = rf_loaded[i][0] if rf_loaded and i < len(rf_loaded) else 100.0 * (n_blocks - i) / n_blocks
        default_nc = int(rf_loaded[i][1]) if rf_loaded and i < len(rf_loaded) else 100000 * (i + 1)
        with c1:
            sr = st.number_input(
                t("block_stress_range", i=i + 1),
                value=float(default_sr),
                key=f"sr_{i}",
            )
        with c2:
            nc = st.number_input(
    ...
    if st.button(t("run_miner_assessment"), type="primary"):
        ...
        if "miner_result" in result:
            ...
            st.markdown(
                ...
                f'<span>D = {mr["total_damage"]:.4f} / {damage_limit}</span></div>',
                unsafe_allow_html=True,
            )
            ...
            # ── Rich Miner Summary Panel (BELOW histogram) ───────────────
...
with tab3:
    ...
    exp_mat = st.selectbox(
        t("material"), FAMILY_KEYS, format_func=format_family, key="exp_mat",
    )
    exp_va = st.checkbox(t("variable_amplitude_loading"), key="exp_va")

    sn_exp = SNCurve(exp_fat, exp_mat, exp_va)
    fig_exp = FatiguePlots.sn_curve_plotly(
        sn_exp,
        title=t("plot_sn_title", fat=sn_exp.fat_class, mat=format_family(sn_exp.material_type)),
        labels=_sn_labels(),
    )
...
with tab4:
    ...
    cat_mat = st.selectbox(
        t("material"), FAMILY_KEYS, format_func=format_family, key="cat_mat",
    )
...
with tab5:
    ...
    st.plotly_chart(fig_haigh, key="haigh_tab5")
